...
const computeEnrollmentWindow = (settings) => {
  ...
  // Next June 1 after close date (for "closed" message)
...
const EnrollmentForm = ({ onClose }) => {
  ...
  const getTuitionKey = (grade) => {
    switch (grade) {
      ...
      case "grade1": case "grade2": case "grade3": return "grade1-3";
      case "grade4": case "grade5": return "grade4-5";
  ...
  const gradeOptions =
    educationLevel === "preschool"
      ? [{ value: "prek", label: "Pre-Kinder" }, { value: "kinder", label: "Kinder" }]
  ...
  const buildAddress = () =>
    [street, barangay, city, province, region, zipCode].map((p) => p.trim()).filter(Boolean).join(", ");

  /* ── Combine name parts into a full name string for the backend ── */
  ...
  const handleSubmit = async (e) => {
    ...
    // Re-check window against backend on submit
    const liveSettings = await fetch(`${API_BASE}/api/enrollment-settings/`).then((r) => r.json()).catch(() => null);
    ...
    if (website && website.trim()) { alert("Invalid submission."); return; }
    if (!gradeLevel) { alert("Please select a Grade Level."); return; }
    ...
    const payload = {
      ...
      academic_year:    academicYear,   // ← comes from backend settings now
      website,
      lrn,
      last_name:        lastName,
      ...
      remarks:          "",
      parent_info: {
        // Full names combined for backend (First Middle Last)
    ...
    <div className="enrollment-container">
      ...
      <form onSubmit={handleSubmit} className="enrollment-form">
        ...
        <div style={{ display: "none" }}>
          <input type="text" name="website" autoComplete="off" value={website} onChange={(e) => setWebsite(e.target.value)} />
        </div>
        ...
        <div className="form-grid">
          ...
          <div className="form-group">
            ...
            <select value={gradeLevel} disabled={!educationLevel} onChange={(e) => setGradeLevel(e.target.value)}>
              <option value="">Select</option>
              {gradeOptions.map((g) => {
                const rule = GRADE_AGE_RULES[g.value];
                return <option key={g.value} value={g.value}>{g.label}{rule ? ` (age ${rule.min}–${rule.max})` : ""}</option>;
              })}
            </select>
          </div>
        </div>
        ...
        {educationLevel && (
          <details className="age-reference">
            ...
            <table className="age-reference__table">
              <thead><tr><th>Grade</th><th>Min Age</th><th>Max Age</th></tr></thead>
              <tbody>
                {gradeOptions.map((g) => {
                  const rule = GRADE_AGE_RULES[g.value];
                  ...
                  return (
                    <tr key={g.value} className={isSelected ? (inRange ? "age-ref-row--valid" : "age-ref-row--invalid") : ""}>
                      <td>{g.label}</td><td>{rule?.min}</td><td>{rule?.max}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div className="form-grid">
          <div className="form-group form-group--full">
            ...
            <input value={street} onChange={(e) => setStreet(e.target.value)} placeholder="e.g. 123 Rizal St." />
          </div>
          <div className="form-group">
            <label>Barangay</label>
            <input value={barangay} onChange={(e) => setBarangay(e.target.value)} placeholder="e.g. Brgy. Santo Niño" />
          </div>
          <div className="form-group">
            <label>City / Municipality</label>
            <input value={city} onChange={(e) => setCity(e.target.value)} placeholder="e.g. Caloocan City" />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <input value={zipCode} onChange={(e) => setZipCode(e.target.value)} placeholder="e.g. 1400" maxLength={4} inputMode="numeric" />
          </div>
        </div>
        {buildAddress() && (
          <div className="address-preview"><span>📌</span><span>{buildAddress()}</span></div>
        )}
        ...
        {/* ── Guardian ── */}
        <p className="parent-section-label">Guardian <span style={{ fontWeight: 400, color: "#9ca3af" }}>(if applicable)</span></p>
        <div className="form-grid">
          <div className="form-group">
            <label>First Name</label>
        ...
        {tuition && (
          <div className="tuition-box">
            ...
            {paymentMode === "cash" && (
              <>
                <div className="tuition-row"><span>Tuition Fee (Cash)</span><span>₱{tuition.cash.toLocaleString()}</span></div>
                <div className="tuition-row"><span>Miscellaneous (August)</span><span>₱{tuition.misc_aug.toLocaleString()}</span></div>
                <div className="tuition-row"><span>Miscellaneous (November)</span><span>₱{tuition.misc_nov.toLocaleString()}</span></div>
                {studentType === "new" && <div className="tuition-row"><span>Assessment Fee</span><span>₱{tuition.assessment.toLocaleString()}</span></div>}
                <div className="tuition-total">
                  <strong>Total (Cash)</strong>
                  <strong>₱{(tuition.total_cash + (studentType === "new" ? tuition.assessment : 0)).toLocaleString()}</strong>
                </div>
              </>
            )}
            {paymentMode === "installment" && (
              <>
                <div className="tuition-row"><span>Tuition Fee (Installment)</span><span>₱{tuition.installment.toLocaleString()}</span></div>
                <div className="tuition-row"><span>Initial Payment</span><span>₱{tuition.initial.toLocaleString()}</span></div>
                <div className="tuition-row"><span>Reservation Fee</span><span>₱2,000</span></div>
                <div className="tuition-row"><span>Monthly Payment</span><span>₱{tuition.monthly.toLocaleString()}</span></div>
                <div className="tuition-row"><span>Miscellaneous (August)</span><span>₱{tuition.misc_aug.toLocaleString()}</span></div>
                <div className="tuition-row"><span>Miscellaneous (November)</span><span>₱{tuition.misc_nov.toLocaleString()}</span></div>
                {studentType === "new" && <div className="tuition-row"><span>Assessment Fee</span><span>₱{tuition.assessment.toLocaleString()}</span></div>}
                <div className="tuition-total">
                  <strong>Total (Installment)</strong>
                  <strong>₱{(tuition.total_installment + (studentType === "new" ? tuition.assessment : 0)).toLocaleString()}</strong>
                </div>
              </>
            )}
          </div>
        ...
        <div className="form-actions">
          <button type="submit" disabled={ageValidation !== null && !ageValidation.ok}>
            Submit Enrollment
          </button>
